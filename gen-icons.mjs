// Generates icons.js from maintained icon packages at build time.
...
const lines = [];
lines.push("// GENERATED by gen-icons.mjs — do not edit directly.");
lines.push("// Sources: simple-icons (brands) + lucide-static (stroke) + icons.fallback.json.");
lines.push("");
lines.push("export const ICONS = {");
for (const [name] of BRANDS) {
  const { color, path } = ICONS[name];
  lines.push(`  ${JSON.stringify(name)}: { color: ${JSON.stringify(color)}, path: ${JSON.stringify(path)} },`);
}
lines.push("};");
lines.push("");
lines.push("export const TAB_ICONS = {");
for (const [id] of TABS) {
  lines.push(`  ${JSON.stringify(id)}: \`${TAB_ICONS[id]}\`,`);
}
lines.push("};");
lines.push("");

writeFileSync(join(here, "icons.js"), lines.join("\n"));
console.log(`gen-icons: wrote icons.js (${Object.keys(ICONS).length} brands, ${Object.keys(TAB_ICONS).length} tabs)`);
